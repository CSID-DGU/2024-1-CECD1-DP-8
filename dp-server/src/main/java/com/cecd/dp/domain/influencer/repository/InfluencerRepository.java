...
public interface InfluencerRepository extends JpaRepository<Influencer, Long> {
    ...
    List<MostPostsProjection> getMostThreePostsById(@Param("id") Long influencerId, Pageable pageable);
    ...
    @Query(value = "SELECT AVG(subquery.like_cnt) "
            + "FROM ( "
            + "    SELECT m.like_cnt "
            + "    FROM Media m "
    ...
    @Query(value = "SELECT AVG(subquery.comments_cnt) "
            + "FROM ( "
