...
public class InfluencerService {
    ...
    public InfluencerService(InfluencerRepository influencerRepository, MetaRepository metaRepository, MetaRepository metaRepository1, MediaRepository mediaRepository) {
    ...
    public GetInfluencerReportDTO getReport(Long influencerId) {
        ...
        // 프로필 정도
        ProfileProjection profile = influencerRepository.getProfileById(influencerId, PageRequest.of(0, 1)).get(0);

        // 인기 게시물 3개
        List<MostPostsProjection> mostPosts = influencerRepository.getMostThreePostsById(influencerId, PageRequest.of(0, 3));

        // 인플루언서 모든 게시물의 모든 해시태그 이름 (DISTINCT)
        List<String> allTagsOfMedias = influencerRepository.getAllTagNamesById(influencerId);

        // 반응 지수
        List<Float> reactionQuotient = influencerRepository.calculateReactionQuotientById(influencerId, PageRequest.of(0, 1));
        ...
        Meta latestMeta = metaRepository.findMetaByInfluencerId(influencerId, PageRequest.of(0,1)).get(0);
