...
# 모든 ground truth 해시태그에 대해 평가 수행
num_hashtags = len(ground_truth)
k = 10

for seed_node in ground_truth.keys():
    if seed_node not in [edge[0] for edge in weighted_edges]:
        print(f"해당 노드가 그래프에 존재하지 않습니다: {seed_node}")
        num_hashtags -= 1
        continue
    ...
    total_prec_at_k += precision_at_k(rwr_result, gt_for_seed, k)
    total_mrr += mean_reciprocal_rank(rwr_result, gt_for_seed)
    total_map += mean_average_precision(rwr_result, gt_for_seed)
    total_ndcg += ndcg(rwr_result, gt_for_seed, k)
...
# 평균 계산
avg_prec_at_k = total_prec_at_k / num_hashtags
avg_mrr = total_mrr / num_hashtags
avg_map = total_map / num_hashtags
avg_ndcg = total_ndcg / num_hashtags
avg_precision = total_precision / num_hashtags
avg_recall = total_recall / num_hashtags
avg_f1 = total_f1 / num_hashtags

print(f"Average Precision@{k}: {avg_prec_at_k:.4f}")
...
# 평가 결과 시각화
metrics = ['Precision@K', 'MRR', 'MAP', 'NDCG@K', 'Precision', 'Recall', 'F1-Score']
values = [avg_prec_at_k, avg_mrr, avg_map, avg_ndcg, avg_precision, avg_recall, avg_f1]

plt.figure(figsize=(10, 6))
plt.bar(metrics, values, color=['blue', 'orange', 'green', 'red', 'purple', 'brown', 'pink'])
plt.xlabel('Metrics')
plt.ylabel('Values')
plt.title('Average Evaluation Metrics for RWR Algorithm')
plt.ylim(0, 1)
plt.show()
